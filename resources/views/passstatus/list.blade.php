...
@section('content')   
	     ...
	     <div class="col-md-6"><h2 class="sub-header pull-left">Pass List</h2>
		 @if(in_array(7,Session::get('log_id')))
		 <a href="{{ url('/passstatus/create') }}" class="create_btn">Create <i class="fa fa-plus" aria-hidden="true"></i></a>
		 @endif
	     </div>
		...
		<?php $i=1; 
		if(count($pass) > 0){
		foreach($pass as $cvalue){  ?>
		
                <tr>
		    <td>{{ $i }}</td>
		    <td>{{ $cvalue->pass_name }}</td>
		    <td>{{ $cvalue->pass_remark }} </td>
			...
			@if(in_array(2,Session::get('log_id')))
			  <a href="{{ url("/passstatus/edit/".$cvalue->id) }}" class="btn btn-primary">Edit</a>
			@endif
			@if(in_array(4,Session::get('log_id')))
			  <span class="display-inline">
			  <form class="form-horizontal" id="em_del_form" role="form" method="POST" action="{{ url('/passstatus/delete') }}">
			      {!! csrf_field() !!}
