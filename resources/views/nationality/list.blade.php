...
@section('content')   
	     ...
	     <div class="col-md-6"><h2 class="sub-header pull-left">Nationality List</h2>
		 @if(in_array(7,Session::get('log_id')))
		 <a href="{{ url('/nationality/create') }}" class="create_btn">Create <i class="fa fa-plus" aria-hidden="true"></i></a>
		 @endif
	     </div>
		...
		foreach($nationality as $cvalue){  ?>
		
                <tr>
		    <td>{{ $i }}</td>
		    <td>{{ $cvalue->nationality_name }}</td>
			...
			@if(in_array(2,Session::get('log_id')))
			  <a href="{{ url("/nationality/edit/".$cvalue->id) }}" class="btn btn-primary">Edit</a>
			@endif
			@if(in_array(4,Session::get('log_id')))
			  <span class="display-inline">
			  <form class="form-horizontal" id="em_del_form" role="form" method="POST" action="{{ url('/nationality/delete') }}">
			      {!! csrf_field() !!}
